// v1.0.0 — HeatmapView: conteúdo reutilizável do mapa de calor
// Usado pela page /heatmap e pelo HeatmapDialog (modal em /orders)
...
interface HeatPoint { lat: number; lng: number; qty: number; }
...
interface HeatmapViewProps {
  compact?: boolean; // modo compacto: pula stats cards + dicas (otimiza espaço em modal)
  mapHeight?: number; // altura do mapa em px
}

export function HeatmapView({ compact = false, mapHeight = 500 }: HeatmapViewProps) {
  ...
  useEffect(() => {
    ...
    const heatData = points.map(p => [
      Number(p.lat),
      Number(p.lng),
      Math.min(Number(p.qty) / 5, 1),
    ]);

    heatLayer.current = (L as any).heatLayer(heatData, {
      radius: 30, blur: 25, maxZoom: 17,
      gradient: {
        0.0: "#3b82f6", 0.3: "#10b981", 0.5: "#f59e0b",
        0.7: "#ef4444", 1.0: "#7c2d12",
      },
    }).addTo(leafletMap.current);
  ...
  return (
    <div className="space-y-4">
      {/* Filtros período + título inline (modo compacto não duplica título do modal) */}
      ...
      {/* Stats compactos em linha quando modal, cards quando page */}
      {compact ? (
        <div className="flex gap-4 text-sm">
          <span><strong className="text-orange-600">{totalOrders}</strong> pedidos</span>
          <span><strong className="text-blue-600">{uniqueLocations}</strong> locais</span>
          <span><strong className="text-green-600">{uniqueLocations > 0 ? (totalOrders / uniqueLocations).toFixed(1) : "0"}</strong> média/local</span>
        </div>
      ) : (
      ...
      <Card>
        {!compact && (
          <CardHeader className="pb-2">
            <CardTitle className="text-base flex items-center gap-2">
              <MapPin className="h-4 w-4" /> Distribuição dos clientes
            </CardTitle>
          </CardHeader>
        )}
        <CardContent className={compact ? "p-2" : ""}>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center text-muted-foreground" style={{ height: mapHeight }}>
              <Flame className="h-12 w-12 opacity-30 mb-2" />
              <p className="text-sm">Sem dados de localização no período</p>
              <p className="text-xs mt-1">Cliente precisa enviar localização via WhatsApp ou cardápio digital</p>
            </div>
          ) : (
            <>
              <div ref={mapRef} className="w-full rounded-lg overflow-hidden border" style={{ height: mapHeight }} />
              <div className="mt-3 flex items-center justify-center gap-3 text-xs text-muted-foreground flex-wrap">
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-blue-500" /> Poucos</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-green-500" /> Médio</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-yellow-500" /> Alto</span>
                <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-red-500" /> Concentração alta</span>
              </div>
            </>
          )}
        </CardContent>
      ...
      {/* Dicas só na page, modal economiza espaço */}
      {!compact && (
        <Card>
